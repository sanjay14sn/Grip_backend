...
export default class ChapterController {
  ...
  async createChapter(
    ...
    try {
      ...
      const existingChapter = await Chapter.findOne({
        chapterName: {
          $regex: new RegExp(`^${chapterData.chapterName}$`, "i"),
        },
        isDelete: 0,
      });
  ...
  @Delete("/:id")
  async deleteChapter(@Param("id") id: string, @Res() res: Response) {
    try {
    ...
    try {
      ...
      for (const chapter of chapters) {
        try {
          ...
          if (
            !chapter?._id ||
            !mongoose.Types.ObjectId.isValid(String(chapter._id))
          ) {
            console.warn(`Invalid chapter ID: ${chapter._id}`);
            continue;
          }

          const chapterObjectId = new mongoose.Types.ObjectId(
            String(chapter._id)
          );
          ...
          console.error(
            `Error processing chapter ${chapter?._id}:`,
            chapterError
          );
          continue;
        }
  ...
  @Post("/headTableMembers/:chapterId")
  @UseBefore(AuthMiddleware)
  async updateHeadTableMembers(
    @Param("chapterId") chapterId: string,
    ...
    try {
      const { president, secretary, vicePresident } = payload;

      // Fetch roles once
      const roles = await Role.find({
        name: { $in: ["President", "Secretary", "Vice President"] },
      });

      const getRoleId = (name: string) =>
        roles.find((r) => r.name === name)?._id || null;

      // Prepare update promises
      const updates = [];

      const upsertHeadTable = async (memberId: string, roleName: string) => {
        ...
        const state = await State.findOne({
          name: { $regex: new RegExp(`^${stateName.trim()}$`, "i") },
        });

        // Find role ID
        const role = await Role.findOne({ name: roleName });

        return HeadTable.findOneAndUpdate(
          { chapterId: memberChapterId, panelAssociateId: memberId },
          {
            countryId: country?._id,
            stateId: state?._id,
            zoneId,
            chapterId: memberChapterId,
            panelAssociateId: memberId,
      ...
      if (president) {
        ...
        await upsertHeadTable(president, "President");
      }
      ...
      if (secretary) {
        ...
        await upsertHeadTable(secretary, "Secretary");
      }
      ...
      if (vicePresident) {
        ...
        await upsertHeadTable(vicePresident, "Vice President");
      }
      ...
      return res.status(200).json({
        success: true,
        message: "Head table member roles saved successfully",
      });
    } catch (error) {
      console.error("🔥 HEADTABLE ERROR:", error);
      return res.status(500).json({
        success: false,
        message: "Failed to update head table roles",
  ...
  async getChapterTopPerformers(
    ...
    try {
            ...
            $project: {
              ...
              name: {
                $concat: [
                  ...
                  " ",
                  "$memberDetails.personalDetails.lastName",
                ],
              },
      ...
      const thankYouSlipTotalAmount =
        thankYouSlipAmountResult.length > 0
          ? thankYouSlipAmountResult[0].totalAmount
          : 0;
      ...
      console.error(
        `Error fetching statistics for chapter ${chapterId}:`,
        error
      );
      return res.status(500).json({
        success: false,
