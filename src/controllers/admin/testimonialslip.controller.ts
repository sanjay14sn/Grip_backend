...
@JsonController("/api/admin/testimonialslips")
@UseBefore(AuthMiddleware)
export default class TestimonialSlipController {


    @Get('/list')
    async listTestimonialslips(@QueryParams() queryParams: ListTestimonialSlipDto, @Res() res: Response, @Req() req: Request) {
        ...
        try {
            ...
            for (const chapter of chapters) {
                ...
                const oneToOneQuery: FilterQuery<ITestimonialSlip> = {
                    isDelete: 0,
                    $or: [
                ...
                const oneToOneRecords = await TestimonialSlip.find(oneToOneQuery).lean();
            ...
            return res.status(200).json({
                success: true,
                message: 'TestimonialSlip records fetched successfully',
                data: responseData,
                pagination: {
            ...
            console.error('Error fetching TestimonialSlip records:', error);
            throw new InternalServerError('Failed to fetch TestimonialSlip records');
        }
    }

    @Get("/member/list/:id")
    async listTestimonialSlipMembers(
        @Param("id") id: string,
        @Req() req: Request,
        ...
        try {
            ...
            return res.status(200).json({
                success: true,
                message: 'TestimonialSlip members records fetched successfully',
                data: {
                    chapter: {
                        _id: chapter._id,
                        chapterName: chapter.chapterName,
    ...
    async updateStatus(
        ...
        try {
            const testimonialSlipRecord = await TestimonialSlip.findById(id);

            if (!testimonialSlipRecord) {
                throw new NotFoundError('Testimonial slip record not found');
            }

            testimonialSlipRecord.status = body.status;
            await testimonialSlipRecord.save();

            return res.status(200).json({
                success: true,
                message: 'Status updated successfully',
                data: testimonialSlipRecord,
            });
        } catch (error: unknown) {
