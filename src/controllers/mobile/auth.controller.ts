...
export default class AuthController {
    ...
    async memberLogin(@Body({ validate: true }) loginData: LoginMemberDto, @Res() res: Response) {
        try {
          ...
          // }
          // STEP 1 — find by mobile only
          const member = await Member.findOne({
            "contactDetails.mobileNumber": loginData.mobileNumber,
            isDelete: 0,
          });
          ...
          if (member.status !== "active") {
            return res
              .status(401)
              .json({ success: false, message: "Active member not found" });
          }
          ...
          return res.status(200).json({
            ...
            member: {
              ...
              username: `${member.personalDetails?.firstName || ""} ${
